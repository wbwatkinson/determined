...
class Timer:
    ...
    def stop(self):
        assert self.t1 is not None, "start time is missing. Did you remember to start() the Timer?"
        self.t2 = time.time()
        print(f'Timer "{self.name}" took {humanize_float(self.t2-self.t1)} seconds')

tall = Timer("all").start()
ti = Timer("imports").start()
# import hashlib
# import os
# import socket
# import ssl
...
# from determined.common.api.request import get
ti.stop()
...
def main(args: List[str] = sys.argv[1:]) -> None:
    t = Timer("cli.main").start()
    # try:
    t1 = Timer("make_parse").start()
    ...
    t1.stop()
        ...
        # t2 = Timer("argcomplete.autocomplete").start()
        # argcomplete.autocomplete(parser)
        # t2.stop()

        # t3 = Timer("parser.parse_args").start()
